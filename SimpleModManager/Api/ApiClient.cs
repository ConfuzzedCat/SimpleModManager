...
public sealed class ApiClient : IDisposable
{
    ...
    public HttpResponseMessage Send(HttpRequestMessage request)
    {
        ...
        _logger.Information("Sending request: {0}", request);
    ...
    {
        EnsureNexusRateLimit(request);
        _logger.Information("Sending request as async: {0}", request);
    ...
    {
        var isNexus = request.RequestUri != null && request.RequestUri.Host.Contains("nexusmods", StringComparison.CurrentCultureIgnoreCase);
        return isNexus;
    }
    ...
    public async Task<ModInfoApi> RequestModInfo(string gameName, int modId)
    {
        ...
        using (var response = await SendAsync(request))
        {
            ...
            return JsonConvert.DeserializeObject<ModInfoApi>(body) ?? throw new Exception("Couldn't deserialize response from NexusMods");
        }
    }

    private void UpdateNexusRateLimitValues(HttpResponseMessage response)
    {
        if (response.RequestMessage != null && !IsRequestToNexus(response.RequestMessage))
        {
            return;
        }
        ...
        if (hourlyRemainingValues.Any(hourlyRemainingValue => int.TryParse(hourlyRemainingValue, out _remainingHourlyRequest)))
        {
            _logger.Debug("Updated hourly nexus rate limit to {0}.", _remainingHourlyRequest);
        }

        if (dailyRemainingValues.Any(dailyRemainingValue => int.TryParse(dailyRemainingValue, out _remainingDailyRequest)))
        {
